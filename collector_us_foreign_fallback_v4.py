...
def normalize_label(value: Any) -> str:
    return _clean_text(value).lower().replace("’", "'").replace("&", "and")
...
def _matches(text: Any, aliases: List[str]) -> bool:
    ...
    return normalized in aliases_n or any(len(a) >= 8 and a in normalized for a in aliases_n)
...
def _metric_by_period(table: pd.DataFrame, metric: str, fiscal_year: Optional[int] = None) -> Dict[str, Any]:
    ...
    pending_negative = False
    raw = []
    for cidx, cell in enumerate([_clean_text(v) for v in table.iloc[idx].tolist()]):
        if not cell:
            continue
        if cell in {"(", "["}:
            pending_negative = True
...
def _balance_metric(table: pd.DataFrame, metric: str, value_index: int = 0) -> Optional[float]:
    idx = _find_row(table, metric)
    if idx is None:
        return None
    vals = _row_numeric_values(table.iloc[idx])
    if len(vals) <= value_index:
        return None
    return vals[value_index]
...
def _section_subtotal(table: pd.DataFrame, section: str) -> Optional[float]:
    ...
    active = False
    for i in range(len(table)):
        first = normalize_label(_clean_text(table.iloc[i, 0]))
        ...
        if first and first.endswith(":") and first != target + ":":
            break
        vals = _row_numeric_values(table.iloc[i])
...
def _balance_snapshot(table: pd.DataFrame) -> Dict[str, Any]:
    ...
    receivables = None if trade_receivables is None and unbilled is None else (trade_receivables or 0.0) + (unbilled or 0.0)
    return {
...
def find_financial_tables(html: str) -> List[Dict[str, Any]]:
    ...
    return [{"table_index": i, "table_type": _classify(t), "table": t} for i, t in enumerate(tables) if _classify(t) != "other"]


def normalize_foreign_result(html: str, ticker: str = "", currency: str = "", scale: str = "", fiscal_end: Optional[str] = None, form: Optional[str] = None) -> Dict[str, Any]:
    ...
    income_out = {m: _metric_by_period(income, m, fiscal_year=fiscal_year) for m in ("revenue", "operating_income", "net_income", "sga", "finance_costs", "eps_basic", "eps_diluted")}
    cf = _metric_by_period(cashflow, "operating_cash_flow", fiscal_year=fiscal_year) if not cashflow.empty else {}
    return {
        "ticker": ticker,
        "currency": currency,
        "scale": scale,
        ...
        "cash_flow": {
            "quarter": {"operating_cash_flow": cf.get("quarter"), "prior": cf.get("prior_by_period", {}).get("quarter")},
            "ytd": {"operating_cash_flow": cf.get("ytd"), "prior": cf.get("prior_by_period", {}).get("ytd")},
            "annual": {"operating_cash_flow": cf.get("annual"), "prior": cf.get("prior_by_period", {}).get("annual")},
        },
    }
...
def build_standard_metric_payload(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ytd_rev, ytd_rev0 = ytd.get("revenue"), ytd.get("_prior_year", {}).get("revenue")
    metrics["revenue_growth"] = _safe_div((ytd_rev - ytd_rev0) if ytd_rev is not None and ytd_rev0 is not None else None, ytd_rev0)
    if metrics["revenue_growth"] is not None:
        metrics["revenue_growth"] *= 100

    eps = ytd.get("eps_diluted") if ytd.get("eps_diluted") is not None else ytd.get("eps_basic")
    eps0 = ytd.get("_prior_year", {}).get("eps_diluted") if ytd.get("_prior_year", {}).get("eps_diluted") is not None else ytd.get("_prior_year", {}).get("eps_basic")
    ...
    if metrics["quick_ratio"] is None and cl is not None:
        metrics["quick_ratio"] = _safe_div((b.get("cash") or 0.0) + (b.get("receivables") or 0.0), cl)

    interest = q.get("finance_costs")
    metrics["interest_coverage"] = _safe_div(op, abs(interest) if interest is not None else None)
    ocf = result.get("cash_flow", {}).get("quarter", {}).get("operating_cash_flow")
    metrics["ocf_ratio"] = _safe_div(ocf, q.get("net_income"))
    metrics["sga_ratio"] = _safe_div(abs(q.get("sga")) if q.get("sga") is not None else None, rev)
