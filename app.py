...
if not stocks:
  st.error("DB에 저장된 종목이 없습니다.")
else:
  stock_options = {
      f"{s['stock_name']} ({s['stock_code']})": s["stock_code"] for s in stocks
  }
  selected_option = st.sidebar.selectbox(
      "분석할 종목을 선택하세요", list(stock_options.keys())
  )
  selected_code = stock_options[selected_option]
  ...
  if data:
    ...
    col2.metric(
        "PER (주가수익비율)", f"{data['per']} 배" if data["per"] else "N/A"
    )
    col3.metric(
        "PBR (주가순자산비율)", f"{data['pbr']} 배" if data["pbr"] else "N/A"
    )
    col4.metric(
        "ROE (자기자본이익률)", f"{data['roe']}%" if data["roe"] else "N/A"
    )
    ...
    with col_score:
      ...
      if score >= 80:
        st.success("🟢 **최우수 (하락장 최적 방어주)**")
      elif score >= 60:
        st.info("🟡 **우수 (평균 이상의 체력)**")
      else:
        st.warning("🔴 **주의 (변동성 장세 주의)**")
