"""US Utility scoring v2.4 (final candidate)."""
from __future__ import annotations
from scoring import evaluate_defense_grade

US_PROFILES = {"utility"}

# Final utility model: remove the duplicate OCF/dividend metric and move
# those 4 points into downturn resilience.
PROFILE_METRICS = {
    "utility": {
        "revenue_growth": 7,
        "eps_growth": 7,
...
def calculate_us_utility_score_v2(metrics: dict) -> dict:
    ...
    for metric, weight in weights.items():
        ...
        if value is None:
            scores[metric]["excluded_from_total"] = True
        else:
    ...
    def sub_total(items):
        return round(
            sum(scores[m]["weighted_score"] for m in items if not scores[m].get("excluded_from_total")) * scale,
            1,
        )

    return {
        "profile": "utility_v2_4",
        "metric_scores": scores,
        "total_score": total,
        "grade": grade,
        "grade_desc": grade_desc,
        "available_weight": available_weight,
        "coverage_pct": round(coverage * 100.0, 1) if total_weight else 0.0,
        "score_cap": cap,
        "missing_metric_count": sum(1 for m in weights if metrics.get(m) is None),
        "sub_scores": {
            ...
            "dividend_safety": sub_total(("dividend_coverage", "dividend_payout")),
            "downturn": sub_total(("downturn_defense",)),
        },
    }

if __name__ == "__main__":
    print("utility_v2_4 weight total:", sum(PROFILE_METRICS["utility"].values()))
