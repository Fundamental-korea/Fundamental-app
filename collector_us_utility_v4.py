...
from us_utility_extraction import REVENUE_TAGS, OPERATING_INCOME_TAGS, NET_INCOME_TAGS, ASSETS_TAGS, EQUITY_TAGS, pick_flow, pick_instant, pick_eps, pick_interest, pick_debt, pick_ocf, pick_capex, pick_dividend, core_years
...
def _suspicious(facts,latest):
    ...
    eps=pick_eps(facts,latest); div=pick_dividend(facts,latest)
    if equity is None or eps is None or div is None or debt is None: return True
    if debt.get("method")=="components" and debt.get("current") and debt.get("noncurrent") is None: return True
    d=debt.get("val")
