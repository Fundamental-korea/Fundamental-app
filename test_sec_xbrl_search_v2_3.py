...
def main():
    print("=" * 100)
    print("SEC XBRL SEARCH V2.3.1 REGRESSION TEST")
    print("=" * 100)

    ticker_map = load_ticker_map()
    print(f"\nSEC ticker map: {len(ticker_map):,} tickers")

    engine = SECXBRLSearchV2_3_1(
    ...
    for ticker, metrics in TEST_CASES.items():
        ...
        try:
            ...
            accession, primary_doc, filed = engine.latest_annual_filing(submissions)
        ...
        for metric in metrics:
            ...
            print(f"\nCompany Facts candidates: {len(company_candidates)}")
            for i, candidate in enumerate(company_candidates[:5], 1):
                print_candidate(candidate, prefix=f"  CF #{i}: ")

            filing_candidates = result.get("filing_xbrl", [])
            print(f"\nFiling XBRL candidates: {len(filing_candidates)}")
            for i, candidate in enumerate(filing_candidates[:5], 1):
                print_candidate(candidate, prefix=f"  FX #{i}: ")
            ...
            if best is None:
                ...
                fy = best.get("fy")
                compact_concept = str(concept).lower().replace(" ", "")
                compact_label = str(label).lower().replace(" ", "")

                if annual_fy is not None and target_year != annual_fy:
                ...
                if fy is not None and target_year is not None:
                    try:
                        if abs(int(fy) - int(target_year)) > 1:
                            reasons.append(f"FY_MISMATCH fy={fy}")
                    except Exception:
                        pass
                ...
                if metric == "liabilities":
                    ...
                    for x in bad:
                        if x in compact_concept or x in compact_label:
                            reasons.append(f"COMPONENT_LIABILITY:{x}")
                ...
                if metric == "interest_expense":
                    ...
                    for x in bad:
                        if x in compact_concept or x in compact_label:
                            reasons.append(f"BAD_INTEREST:{x}")
                ...
                if metric == "inventory":
                    ...
                    for x in bad:
                        if x in compact_concept or x in compact_label:
                            reasons.append(f"COMPONENT_INVENTORY:{x}")

                if metric == "operating_income":
                    bad = ["nonoperating", "othernonoperating", "nonoperatingincomeexpense"]
                    for x in bad:
                        if x in compact_concept or x in compact_label:
                            reasons.append(f"NONOPERATING_INCOME:{x}")

                if metric == "current_assets" and "noncurrent" in compact_concept:
                    reasons.append("NONCURRENT_ASSET")

                if metric == "current_liabilities" and "noncurrent" in compact_concept:
