"""Incremental SEC filing updater for the US Standard scoring universe.

Checks SEC submissions for all eligible companies whose scoring_profile is
"standard" and only re-collects companies that already have a stored snapshot
and whose latest relevant filing is newer than that snapshot.

Initial collection is intentionally kept separate. This prevents a manual
or scheduled run from accidentally launching a bootstrap of the entire
Standard universe.
"""
...
def _load_universe(sb):
    """Load the full eligible Standard scoring universe."""
    rows = []
    offset = 0
...
def main():
    ...
    if not SEC_USER_AGENT or "contact@example.com" in SEC_USER_AGENT:
        print(
            "[WARN] SEC_USER_AGENT is using the collector default. "
            "Set a real contact email in GitHub Secrets."
        )

    sb = create_client(SUPABASE_URL, SUPABASE_KEY)
    universe = _load_universe(sb)
    existing = _load_existing(sb)
    print(f"[UNIVERSE] Standard scoring eligible companies: {len(universe)}")
        ...
        try:
            ...
            if _needs_update(latest, stored):
                ...
                print(
                    ...
                    f"form={latest.get('form')} "
                    f"report={latest.get('report_date')}"
                )
    ...
    if not changed:
        if errors:
            raise RuntimeError(
                f"SEC filing check completed with {errors} errors and no updates"
            )
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"collector_us_standard_fallback.py exited with {result.returncode}"
        )
