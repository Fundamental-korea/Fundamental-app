"""Dry-run comparison: current US utility scoring vs Utility v2.4.
No Supabase writes. Fetches SEC Company Facts and compares the current model
with the proposed v2.4 utility model.
"""
...
def dividend_amount(facts, year):
    v = value(facts, year, DIVIDEND_TAGS)
...
def dividend_safety_metrics(facts, year):
    ...
    dividend_coverage = abs(ocf) / dividend if dividend and dividend > 0 and ocf is not None else None
    payout = dividend / net_income * 100.0 if dividend and dividend > 0 and net_income and net_income > 0 else None
    return {"dividend_coverage": dividend_coverage, "dividend_payout": payout, "dividend": dividend, "ocf": ocf, "net_income": net_income}
...
def main():
    ...
    print("UTILITY V2.4 DRY RUN — NO DB WRITE")
    print("Weights: Revenue 7 | EPS 7 | OPM 10 | ROA 8 | Debt/Capital 15 | OCF/Debt 10 | FCF/Debt 8 | Interest 10 | Dividend Coverage 6 | Payout 3 | Downturn 16")
    print()
    for ticker in tickers:
        try:
            ...
            dm = dividend_safety_metrics(facts, latest_year)
            metrics.update({k: dm[k] for k in ("dividend_coverage", "dividend_payout")})
            ...
            print(f"  V2.4    : {v2['total_score']:.1f} {v2['grade']} | coverage={v2['coverage_pct']:.1f}% | missing={v2['missing_metric_count']}")
            print(f"  CHANGE  : {v2['total_score'] - old['total_score']:+.1f}")
            print(f"  DIV DATA: dividend={dm['dividend']!r} ocf={dm['ocf']!r} net_income={dm['net_income']!r}")
            for metric, entry in v2["metric_scores"].items():
                old_entry = old["metric_scores"].get(metric)
                old_score = old_entry["score"] if old_entry else None
                print(f"    {metric:20s} value={entry['value']!r:>12} old={old_score!s:>4} new={entry['score']:>4} v2.4_weight={entry['weight']:>2} contrib={entry['weighted_score']:>5.2f}")
            print()
